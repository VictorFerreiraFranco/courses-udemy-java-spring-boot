...
public class Car {
    ...
    public CarStatus ignite(Key key) {
        if (key.getAutoMaker() != this.getAutoMaker()) {
            return new CarStatus("Cannot start with this key!");
        }

        return new CarStatus("Car started! Running with the engine: " + this.motor);
    }
}
